...
// Ganti declare global yang lama dengan ini:
...
type BandProps = Required<Pick<LanyardProps, "imageFit" | "lanyardWidth">> &
  Pick<LanyardProps, "frontImage" | "backImage" | "lanyardImage"> & {
    maxSpeed?: number;
    minSpeed?: number;
    isMobile?: boolean;
  };
...
type MeshLineHandle = THREE.Mesh & {
...
function Band({
  ...
  const band  = useRef<MeshLineHandle>(null);
  ...
  const segmentProps = {
    ...
    canSleep: true,
    colliders: undefined as unknown as false,
    angularDamping: 4,
    linearDamping: 4,
  };
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  useSphericalJoint(j3 as any, card as any, [[0, 0, 0], [0, 1.5, 0]]);
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && j1.current && j2.current && j3.current && card.current && band.current) {
      ...
      const body1 = j1.current as RapierRigidBody & { lerped: THREE.Vector3 };
      const body2 = j2.current as RapierRigidBody & { lerped: THREE.Vector3 };
      curve.points[0].copy(j3.current.translation());
      curve.points[1].copy(body2.lerped);
      curve.points[2].copy(body1.lerped);
